...
const setName = (req, res) => {
  ...
  if (req.body.animal === 'dog') {
    ...
    // Make sure there isn't already a dog with the name before trying to save
    // Or mongo will be very mad
    ...
    // errs, handle them
      ...
      // If one exists, send back an error
      return res.status(400).json({ id: 'Bad Request', error: 'This dog already exists' });
    });
  }
  // Handle for cats
...
const searchName = (req, res) => {
  ...
  if (!req.query.name) {
    ...
    // errs, handle them
      ...
      // if a match, send the match back after increasing the age
      const newDoc = doc;
      newDoc.age += 1;
